    ...
    public class Simulation : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (!Paused)
            {
                ...
                {
                    Vehicle v = Vehicles[x];
                    Objects.RemoveAt(x+ 1 * Convert.ToInt32(pedestrian.Exists));
                    v.LiDAR = new List<Vector2[]>(Objects);
                    Objects.Insert(x+ 1 * Convert.ToInt32(pedestrian.Exists), v.GetBoundingBox());
                    v.Update(gameTime);
                }
            ...
            if (Vars.ElapsedGameTime > 0)
            {
                ...
                if (Hidden == 1)
                {
                    ...
                    if (Vehicles.Count > 0 && Vars.SelectedVehicleID != 0)
                    {
                        ...
                        spriteBatch.DrawString(font, "Interesction Distance: " + Math.Round(HoveredVehicle.IntersectionDistance), new Vector2(20, height), Color.Red); height += 50;
...
#pragma warning disable CS0618 // Type or member is obsolete
            ...
            if (Vars.MaxNumberOfVehicles > 0)
            {
                for (int i = 0; i < Vehicles.Count; i++)
                {
                    ...
                    {
                        spriteBatch.Draw(Vehicle.Sprite, Vehicles[i].Position, null, null, Vehicle.Origin, (float)-Vehicles[i].Velocity[1], new Vector2(Vehicle.Xscale, Vehicle.Yscale), Color.DarkSlateGray);
                    }
                    else
                    {
                        spriteBatch.Draw(Vehicle.Sprite, Vehicles[i].Position, null, null, Vehicle.Origin, (float)-Vehicles[i].Velocity[1], new Vector2(Vehicle.Xscale, Vehicle.Yscale), Vehicle.ModelColors[Vehicles[i].ModelNumber]);
                    }
    ...
    public class Pedestrian
    {
        ...
        public void Move()
        {
            if (Dist > Vars.StreetWidthPixels*2)
            {
                Exists = false;
            }
            else
            {
                if (DirectionNumber == 1)  // norf
                {
                    double dx = +(float)(StreetSide * Methods.KmhToMs(Speed) * Vars.PPM * Vars.DeltaTime);
                    Dist += Math.Abs((float)(StreetSide * Methods.KmhToMs(Speed) * Vars.PPM * Vars.DeltaTime));
                    Position.X += (float)(StreetSide * Methods.KmhToMs(Speed) * Vars.PPM * Vars.DeltaTime);
                }
                else if (DirectionNumber == 2)  // east
                {
                    double dy = -(float)(StreetSide * Methods.KmhToMs(Speed) * Vars.PPM * Vars.DeltaTime);
                    Dist += Math.Abs((float)(StreetSide * Methods.KmhToMs(Speed) * Vars.PPM * Vars.DeltaTime));
                    Position.Y -= (float)(StreetSide * Methods.KmhToMs(Speed) * Vars.PPM * Vars.DeltaTime);
                }
                else if (DirectionNumber == 3)  // south
                {
                    double dx = -(float)(StreetSide * Methods.KmhToMs(Speed) * Vars.PPM * Vars.DeltaTime);
