...
namespace Simulation_V2
{
    public class Vehicle
    {
        ...
        public List<Vehicle> LidarVehicles = new List<Vehicle>();

        // constants and information and details
        public static string[] ModelNames = new string[] { "Sedan", "Pickup Truck", "Smart Car", "Sports Coupe", "Crossover", "SUV", "Van" };
        ...
        public static double[,] ModelMultipliers = new double[,] { { 1, 1, 1, 1 }, { 1.25, 1.1, .6, .6 }, { .5, .75, .75, .5 }, { .85, 1, 1.25, 1.25 },{1.1, 1, .9, .9},{ 1.1, 1.1, .8, .8 },{ 1.25, 1.1, .5, .5 } };
        public static string[] ModelSpriteFileNames = new string[] {"white_car", "white_car" , "white_car" , "white_car" , "white_car" , "white_car" , "white_car" };
        ...
        public static Texture2D[] ModelSprites = new Texture2D[7];
        ...
        public Vehicle(int id, int dn = 0, int lane = 0, double speed=0)
        {
            ...
            Sprite = ModelSprites[ModelNumber];
        ...
        private double ClosestObjectDistanceAhead(double maxangle)
        {
            ...
            foreach (Vector2[] boundingbox in LiDAR)
            {
                foreach (Vector2 p in boundingbox)
                {
                    ...
                    // if bounding box is a pedestrian
                    else if (boundingbox.Length == 9 && DestinationNumber != 1)
                    {
                        if ((Methods.Distance(this.Center, p) < dist || dist == 0)&&Methods.Distance(p, this.Center) < Methods.Distance(p, this.BottomCenter))
                        {
                            if (DestinationNumber==0)  // turning left
                            {
        ...
        private double NewLiDAR(double maxangle)
        {
            double dist = 0;
            List<Vector2> Objects = new List<Vector2>();
            foreach(Vector2[] Object in Simulation.Objects)
            {
                foreach(Vector2 point in Object)
                {
                    Objects.Add(point);
                }
            }
            for (int i = 0; i < Simulation.Vehicles.Count; i++)
            {
                foreach (Vector2 point in Simulation.Vehicles[i].GetBoundingBox())
                {
                    if(Simulation.Vehicles[i].ID!=this.ID)
                        Objects.Add(point);
                }
            }
            Methods.Print("Vehicle " + ID + "'s objects list length: " + Objects.Count);
            foreach (Vector2 p in Objects)
            {
                ...
                if ((Math.Atan(dy / dx) - ((Math.PI / 2) * (DirectionNumber % 2))) > -(maxangle * Vars.DegtoRad) && (Math.Atan(dy / dx) - ((Math.PI / 2) * (DirectionNumber % 2))) < (maxangle * Vars.DegtoRad))// && (Methods.Distance(this.Center, p) < dist || dist == 0) && Methods.Distance(p,this.TopCenter) < Methods.Distance(p, this.BottomCenter));     
                {
                    if (Methods.Distance(this.Center, p) < dist || dist == 0)
                    {
                        if (Methods.Distance(p, this.TopCenter) < Methods.Distance(p, this.BottomCenter)-Height*.4)
                            dist = Methods.Distance(this.Center, p);
                    }
                }
            }
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (ProcessorCycleCount >= 60.0 / Vars.VehicleProcessorCyclesPerSec)
            {
                ...
                ObjectAheadDistance = NewLiDAR(1+Velocity[0]*.5);
                ...
                if (ObjectAheadDistance < FollowingDistance && ObjectAheadDistance > -2)
                {
                    ...
                    if (CarAhead.ID != 0 && !InIntersection)//&& Methods.AreSimilar((this.IntersectionDistance-CarAhead.IntersectionDistance + Vehicle.Height) /Vars.PPM, ObjectAheadDistance, (Vehicle.Height/Vars.PPM)))
                    {
                        ...
                        ThrottleCommand = (ObjectAheadDistance - FollowingDistance) / (FollowingDistance );
                    }
                ...
                else  // get acceleration / deceleration from AI / intercommunication 
                {
                    if (ThrottleModification == 0)  
                    {
                        ...
                        {
                            if (IntersectionDistance < FollowingDistance*1.5 && !InIntersection && false)
                            {
                                if (DestinationNumber == 0)
                                    ThrottleCommand = (Maps.SpeedLimit * .75 - Velocity[0]) / 10.0;
                                else if (DestinationNumber == 2)
                                    ThrottleCommand = (Maps.SpeedLimit * .4 - Velocity[0]) / 10.0;
                            }
                            else
